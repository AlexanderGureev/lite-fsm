...
describe("MachineManager actors — effects + sugar для transition", () => {
  ...
  it("ошибки actor effect сообщаются через onError, actor не переходит в terminal", async () => {
    const onError = vi.fn();
    const actorMachine: ReturnType<typeof createLikeSync> = {
      ...createLikeSync(),
      effects: {
...
describe("MachineManager actors — owner state у domain createEffect", () => {
