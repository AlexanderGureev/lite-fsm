...
- Если файл с логикой растет к 800+ строкам, разбивай его на модули с четкими владельцами и тонким entrypoint.
...
- Для расширяемой логики заранее выделяй явные pipeline/rules/registry-функции вместо монолитного orchestrator-а.
- Комментарии должны объяснять неочевидное намерение или ограничения, а не пересказывать код.

## Стиль документации

- Документацию, README и cheatsheets пиши на профессиональном русском: коротко, точно и без разговорных оборотов, метафор и расплывчатых слов.
- Не склоняй английские API-термины через дефис. Используй русский термин (`менеджер`, `снимок`, `контекст`, `запись`) или оставляй точный API identifier в `code`.
- Описывай поведение как контракт: что делает API, что не делает, какие есть ограничения и когда выбирать каждый режим.
...
- Для чистой логики и модулей с контрактами без сайд-эффектов требуй строгий 100% coverage по statements/branches/functions/lines.
