import type { FSMEvent, StateType } from "@lite-fsm/core";
...
const createEntry = (request: ListRequest): EntityListEntry => ({
...
const applyLoadedPage = (entry: EntityListEntry, page: LoadedWidgetPage): EntityListEntry => ({
  ...entry,
  status: "idle",
...
export const entityList = createMachine({
  ...
  reducer: (state, action, { nextState }) => {
    ...
    switch (action.type) {
      case "INITIAL_ENTITY_LIST_DATA": {
        const seed = action.payload;
        state.context.lists[getListId(seed.request)] = applyLoadedPage(createEntry(seed.request), seed);
        return;
      }
