...
> Раздел перенесён в [Пакеты / @lite-fsm/persist](/packages/persist). Эта страница оставлена для старых внешних ссылок.
...
В <a href={playgroundExamplePath("persist")} style={{ color: "#2563eb", textDecoration: "underline", textUnderlineOffset: "2px" }}>демонстрационном примере persist</a> история чата сохраняется в `localStorage`, а другие вкладки получают её через событие `storage`. В хранилище попадает только машина `chatThread`: машина ввода (`chatComposer`) и сессия (`chatSession`) используются только в текущей вкладке.
