...
Каждый dispatch принимает только одно active поле адресации: `actorId`, `groupId` или `groupTag`. Каждое поле принимает строку или массив строк, дубликаты внутри массива удаляются. Если событие содержит несколько полей адресации одновременно, `manager.transition(...)` бросает `LITE_FSM_AMBIGUOUS_ROUTE_META`.
