...
`<project-name>` должен быть относительным путём внутри текущей директории. Родительская директория должна существовать, а целевая директория — отсутствовать.

## Опции create

| Опция                                      | Обязательна | Назначение                                                                 |
| ------------------------------------------ | ----------- | -------------------------------------------------------------------------- |
| `<project-name>`                           | Да          | Относительная директория проекта, например `my-app` или `apps/demo`        |
...
- имя проекта является абсолютным путём, содержит `..` или указывает на уже существующий target;
