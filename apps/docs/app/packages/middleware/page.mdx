...
`@lite-fsm/middleware` расширяет функциональность `MachineManager`: подключает Immer, отправляет события в Redux DevTools, добавляет логирование, аналитику, персистентность и другие промежуточные обработчики.
