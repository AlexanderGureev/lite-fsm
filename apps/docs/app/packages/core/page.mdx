...
```ts copy
import { MachineManager, createEffect, createMachine, defineMachine, definePlugin } from "@lite-fsm/core";
```
...
| `definePlugin`    | Объявляет plugin value для расширения текущего `MachineManager`                                 |
...
| types             | `FSMEvent`, `MachineConfig`, `MachinesState`, `MachineEvents`, plugin/snapshot/actor/middleware types |
...
- Plugin system: `definePlugin`, `MachineManager(..., { plugins })`, storage runtimes, scoped deps, manager extensions и `snapshot.storage[kind]`.
