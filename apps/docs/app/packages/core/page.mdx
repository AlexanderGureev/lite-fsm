...
| Plugin system     | Декларативные sections через `definePlugin().create(...)` и advanced storage runtimes           |
...
- Plugin system: финальный краткий контракт описан в repository cheatsheets; полная документация сайта выполняется отдельно.
