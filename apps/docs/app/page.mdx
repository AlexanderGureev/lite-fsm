...
```bash copy
pnpm add @lite-fsm/core @lite-fsm/middleware
```
...
> **Playground**: Живые примеры с исходниками — sync-машины, async-эффекты, actor-шаблоны и SSR-streaming — доступны в [интерактивном playground](/playground/). Их FSM-графы можно открыть в [визуализаторе](/visualizer/).
...
- [Сохранение состояния](/guide/hydration): SSR, snapshot, hydrate/dehydrate и `FSMHydrationBoundary`
...
- [Persist](/persist): Долговременное хранение состояния `MachineManager` и синхронизация между вкладками
...
- [Visualizer](/visualizer/): Визуальный разбор FSM-графов из JSON IR
