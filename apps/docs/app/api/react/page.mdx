...
`persist` — структурный контракт `{ start(): () => void }`, поэтому `@lite-fsm/react` не зависит от `@lite-fsm/persist`. Если передан ровно один объект с `getStatus()` и `subscribeStatus()`, хуки `@lite-fsm/persist/react` могут читать статус из контекста провайдера без явного контроллера. На сервере провайдер без `persist` отдаёт этим хукам idle fallback; на клиенте без persist с источником статуса они продолжают бросать ошибку провайдера.
