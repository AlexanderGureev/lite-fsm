...
| Plugin system           | `definePlugin().create(...)`, `defineStorageRuntime().create(...)` |
...
| `plugins?`                 | Tuple values из `definePlugin().create(...)`; расширяет типы текущего manager |
...
Публичный plugin объявляется через `definePlugin<PluginEvents, HostEvents>().create(...)`.
`MachineManager(..., { plugins })` принимает только values из builder API и расширяет типы текущего manager по переданному tuple.

Краткий контракт финального API описан в repository cheatsheets. Полная документация сайта для plugin system выполняется отдельной задачей.
