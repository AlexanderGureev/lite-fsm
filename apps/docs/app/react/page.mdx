...
React-интеграция публикуется отдельным workspace/package entrypoint:
...
Если после применения snapshot нужно отправить событие в manager, передайте `transitionAfterHydrate`. Это может быть одно событие или массив событий.
...
`transitionAfterHydrate` выполняется только в браузере, после `manager.hydrate()`. Повторный mount в React StrictMode не отправляет те же события повторно для того же snapshot.
