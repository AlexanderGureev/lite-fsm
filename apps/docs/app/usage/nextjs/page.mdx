...
- **Хранилище должно содержать только мутабельные данные** - `lite-fsm` следует использовать только для тех данных, которые предназначены быть глобальными и изменяемыми.
...
Для Next.js необходимо включить провайдер в корневой layout файл внутри `body`, чтобы корневой `html` оставался серверной частью layout:
...
Когда серверу нужно подготовить состояние и доставить его клиенту, используйте пару `manager.dehydrate()` / `useHydrateSnapshot` — клиент применит снапшот синхронно при первом рендере, без ошибок гидратации. Подробности — в разделе [Сохранение состояния](/guide/hydration).
...
export default async function Page() {
  ...
  return (
    <StoreProvider snapshot={manager.dehydrate()}>
      <PageContent />
    </StoreProvider>
  );
}
...
import { useRef, type ReactNode } from "react";
import { FSMContextProvider, useHydrateSnapshot } from "@lite-fsm/react";
import type { Snapshot } from "@lite-fsm/core";
...
export default function StoreProvider({ children, snapshot }: Props) {
  const storeRef = useRef<AppStore | null>(null);
  if (!storeRef.current) storeRef.current = makeStore();

  useHydrateSnapshot(storeRef.current, snapshot);
