...
`useHydrateSnapshot()` применяет снимок только в `useLayoutEffect`. Используйте его для фонового восстановления после монтирования; он не устраняет SSR hydration mismatch на первой отрисовке. Подробности — в разделе [Сохранение состояния](/guide/hydration).
