...
Persist можно создавать вместе с manager без ручного `typeof window`: `createJsonStorage` принимает lazy factory и не вызывает её во время создания adapter-а или server prerender. `FSMContextProvider` запускает persist lifecycle только после mount.
...
export type PersistentStore = ReturnType<typeof makePersistentStore>;
```
...
`useIsPersistRestoring()` проверяет только фазу `"restoring"`. Если экран должен скрывать данные до завершения первого restore, используйте `status.phase === "idle" || status.phase === "restoring"`.
