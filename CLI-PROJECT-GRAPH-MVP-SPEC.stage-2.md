...
4. Cross-package compatibility фиксируется committed fixture:
...
2. `real-store-shape` fixture пишет graph JSON с local machines и warning для
   unresolved external spread;
3. `--out` обязателен;
4. missing explicit `--tsconfig` блокирует command;
5. missing nearest tsconfig дает info и не блокирует command;
6. выбранный tsconfig резолвит app-local alias внутри fixture root;
7. write failure возвращает `LFC_WRITE_FAILED`;
8. unknown option возвращает `LFC_INVALID_OPTIONS`;
9. generated JSON парсится как `lite-fsm.project-graph-export/v1`;
10. generated `real-store-shape` JSON deep-equals canonical fixture
...
11. playground example entries из manifest пишут graph JSON без blocking
