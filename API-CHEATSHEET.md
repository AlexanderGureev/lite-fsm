...
Ошибки из `intercept`, hooks и guarded storage callbacks пробрасываются из `manager.transition(...)` и не вызывают `onError` автоматически. `ctx.reportError(error)` вызывает текущий `onError` и не меняет control flow. Replacement action из plugin или storage callback валидируется до route recalculation: это должен быть object с `type: string`; `@@lite-fsm/*` запрещен и бросает `LITE_FSM_INVALID_REPLACEMENT_ACTION`. Diagnostics для replacement указывают источник: `plugin '<name>' intercept`, `storage runtime '<kind>' prepareAction` или `storage runtime '<kind>' beforeReduce`.
