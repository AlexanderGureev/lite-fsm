...
`manager` объявляет manager extensions. Ключ section становится полем returned manager, тип поля равен return type factory. Factory получает `ManagerRuntimeContext<PluginEvents>`, поэтому `ctx.transition(...)` принимает события plugin, а не `HostEvents` или произвольный `AnyEvent`. Factory вызывается один раз при создании manager после compile templates, runtime state и initial public state.
