...
`PluginManagerExtensions<Plugins>` остается helper type с одним input и возвращает поля manager по return type factory из section `manager`. Factory получает `ManagerRuntimeContext<PluginEvents>`: `ctx.transition(...)` принимает `ManagerAction<PluginEvents>` и возвращает `ManagerAction<PluginEvents>`. `HostEvents` не входят в этот contract, а `definePlugin().create({ manager })` без первого generic не получает fallback на произвольный `AnyEvent`.
