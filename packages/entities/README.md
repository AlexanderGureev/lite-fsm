...
`reactions` объявляются по типу события и выполняются после коммита транзакции. Они предназначены для синхронизации с внешними системами и не могут диспатчить события. В аргументах: прикладные зависимости плюс `action` и `self`. Кросс-машинные колонки читаются через `entities()` (если он передан в зависимости).
