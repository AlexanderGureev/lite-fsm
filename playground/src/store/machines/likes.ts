  ...
  reducer: (state, action, { nextState }) => {
    ...
    switch (action.type) {
      ...
      case "DISLIKE": {
        ...
        if (item.current === "like") {
          item.likes -= 1;
        }

        if (item.current === "dislike") {
          item.dislikes -= 1;
        }
      ...
      case "SYNC_LIKE_STATE_RESOLVE": {
        ...
        if (item.committed === "like") {
          item.committedLikes -= 1;
        }

        if (item.committed === "dislike") {
          item.committedDislikes -= 1;
        }
