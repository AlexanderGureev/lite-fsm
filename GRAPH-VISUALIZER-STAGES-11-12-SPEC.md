...
`12b`-`12f` должны опираться на эти boundaries и не переопределять их.
...
1. Каждый подэтап `12a`-`12f` закрывается только вместе с тестами для всех
...
3. `12c`: Source editor, `Open visualizer`, controlled diagnostics, reset to
   sample, source edit invalidation и console channel visibility.
4. `12d`: все L1/L2 user flows из проверки подэтапа, включая search/filter,
...
5. `12e`: все L3/simulation user flows из проверки подэтапа, включая open from
...
6. `12f`: regression matrix для полного пути `source -> L1 -> L2 -> L3 ->
...
Этап 12 реализуется как шесть законченных подэтапов. Каждый подэтап должен
...
18. E2E-инфраструктура 12a должна быть достаточной, чтобы 12b-12f добавляли
    новые UI/e2e сценарии без изменения базовой Playwright-конфигурации.
...
12. Каждый подэтап `12a`-`12f` закрыт собственными unit/coverage/e2e tests для
    всех добавленных behavior paths; UI cases покрыты Playwright e2e в том же
    подэтапе, где они появились.
