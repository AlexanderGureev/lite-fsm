...
Заменяет root reducer менеджера автоматов. Этот метод используется middleware для модификации обработки обычных transition, Immer-интеграции и DevTools-style restore.

Если reducer возвращает trusted lite-fsm/devtools/dehydrate-compatible actor records, manager синхронизирует actor sidecar через replacement reconcile. Ручная сборка actor records не является гарантированным API: невалидные slices по-прежнему бросают `LITE_FSM_INVALID_ACTOR_SLICE`.
...
**Пример DevTools-style restore через `replaceReducer`:**

```ts showLineNumbers copy
const restoreMiddleware = (api) => {
  api.replaceReducer((reducer) => (state, action) => {
    if (!action.type.startsWith("@devtools/")) {
