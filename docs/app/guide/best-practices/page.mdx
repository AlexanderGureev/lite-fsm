...
### Используйте константы для имен состояний и событий

Вместо строковых литералов используйте константы для состояний и типов событий:
...
const machine = createMachine({
  config: {
    ...
    [STATES.LOADING]: {
      [EVENTS.FETCH_SUCCESS]: STATES.SUCCESS,
      [EVENTS.FETCH_ERROR]: STATES.ERROR,
    },
    // ...
  },
...
Это делает код более надежным, позволяет избежать опечаток и упрощает рефакторинг.
...
### Используйте фабрики для создания автоматов

Для переиспользования логики создавайте фабрики автоматов:

```ts
// Фабрика для создания автомата загрузки данных
function createDataFetchMachine(options = {}) {
  return createMachine({
    ...
    effects: {
      LOADING: async ({ transition, services }) => {
        try {
          // Используем переданный сервисный метод или общий метод
          const fetchMethod = options.fetchMethod || "fetchData";
          const resource = options.resource || "";

          const data = await services.api[fetchMethod](resource);
...
// Использование фабрики для разных типов данных
const usersMachine = createDataFetchMachine({
  resource: "users",
  fetchMethod: "fetchUsers",
});

const productsMachine = createDataFetchMachine({
  resource: "products",
  fetchMethod: "fetchProducts",
  extraContext: {
    categories: [],
    filters: { price: null, category: null },
  },
});
```
