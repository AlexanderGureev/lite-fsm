...
- [Работа с TypeScript](/guide/advanced-concepts/typescript) - подробное руководство по типизации автоматов и созданию строго типизированного приложения с использованием предварительно типизированных функций API.
- [Интеграция с Next.js](/guide/advanced-concepts/nextjs) - руководство по использованию lite-fsm в Next.js приложениях с App Router.

## Типизация с TypeScript

`lite-fsm` предоставляет богатые возможности для работы с TypeScript и обеспечения типобезопасности. В [отдельном разделе](/guide/advanced-concepts/typescript) мы подробно рассматриваем, как создать предварительно типизированные версии функций и хуков для автоматического применения ваших пользовательских типов.

Вкратце, этот подход позволяет:

1. Создать строго типизированные версии функций API
2. Объединить типы всех автоматов в центральном месте
3. Автоматически получать проверку типов во время компиляции
4. Иметь автодополнение в IDE для состояний, событий и контекста

## Иммутабельное обновление с Immer

Для упрощения обновления состояний `lite-fsm` предоставляет интеграцию с библиотекой Immer, которая позволяет писать "мутирующий" код, который под капотом преобразуется в иммутабельные обновления:
...
const todosMachine = createMachine({
  ...
  reducer: (state, action, { nextState }) => {
    ...
    // Используем Immer для простого обновления состояния
