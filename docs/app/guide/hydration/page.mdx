...
Эта страница описывает, как сохранить состояние `MachineManager` и восстановить его позже: на клиенте после ответа сервера, в другой вкладке, после перезагрузки страницы или из сохраненного черновика.
...
> Если нужно не просто описать снимок, а ещё и автоматически сохранять его в `localStorage`, `IndexedDB` или другое хранилище и синхронизировать между вкладками, посмотрите модуль [Persist](/persist) — он построен поверх `dehydrate`/`hydrate`.
