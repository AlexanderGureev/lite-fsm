...
`FSMHydrationBoundary` применяет snapshot к React subtree так, чтобы `useSelector()` внутри boundary видел hydrated state уже на render, а сам manager догонял это состояние в layout effect.

```tsx showLineNumbers copy
import { FSMHydrationBoundary } from "lite-fsm/react";

function App({ manager, snapshot }) {
  return (
    <FSMContextProvider machineManager={manager}>
...
Подробное описание `hydrate`, `dehydrate`, `getHydratedState` и поведения boundary находится в разделе [Сохранение состояния](/guide/hydration).
